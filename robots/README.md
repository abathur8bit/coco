...
**Running** on the command prompt, navigate to the project folder, then run **robots.exe**:

```
cd c:\workspace\robots
out\build\x86-Debug\robots.exe
```
...
```
perl -e 'print chr(255) x (35*18*256)' > robots.dsk
```
...
```
cmoc robots.c
mv robots.bin minef.bin
writecocofile robots.dsk minef.bin
```
...
```
.\mame64.exe -window coco3 -flop1 robots.dsk -autoboot_delay 1 -autoboot_command "LOADM\"minef\":EXEC\n"
.\mame64.exe -window coco3 -flop1 robots.dsk -autoboot_delay 1 -autoboot_command 'LOADM\"minef\":EXEC\n'
```
